...
const CODE = `import { createEffect, createSignal, onMount } from 'solid-js';
import { FastCommentsCommentWidget, type FastCommentsCommentWidgetHandle } from 'fastcomments-solidjs';
import type { FastCommentsSSO } from 'fastcomments-typescript';

export default function SecureSSO() {
  const [sso, setSSO] = createSignal<FastCommentsSSO>({
    loginURL: 'https://example.com/login',
    logoutURL: 'https://example.com/logout'
  });

  let handle: FastCommentsCommentWidgetHandle | undefined;
  createEffect(() => handle?.update({ sso: sso() }));

  onMount(async () => {
    const info = await fetch('/sso-user-info').then((r) => r.json());
    setSSO((s) => ({ ...s, ...info }));
  });
...
export default function SecureSSO() {
  ...
  createEffect(() => handle?.update({ sso: sso(), hasDarkBackground: isDark() }));
    ...
    <DemoChrome
      ...
      subtitle={<>Production-grade identity. Your server HMAC-signs a base64 user payload; the widget verifies it
        ...
        <a
          ...
          style={{ color: 'var(--fc-accent-c)', 'text-decoration': 'underline dashed' }}
        ...
        <div class="fc-stage__panel" style={{ 'border-color': 'rgba(255, 59, 48, 0.5)', background: 'rgba(255, 59, 48, 0.05)' }}>
          <div style={{ 'font-family': 'var(--fc-mono)', 'font-size': '13px', color: '#ff8b85' }}>
            ...
            <span style={{ color: 'var(--fc-ink-mute)' }}>
              Run <code>node-express</code> from <a href="https://github.com/fastcomments/fastcomments-code-examples" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--fc-accent-c)', 'text-decoration': 'underline dashed' }}>fastcomments-code-examples</a> on port 3003 to continue.
            </span>
          </div>
        </div>
      </Show>
